...
describe('ConsoleLogsTool', () => {
  ...
  test('should retrieve console logs with type filter', async () => {
    ...
    expect((result.content[0] as { text: string }).text).toContain('Retrieved 1 console log(s)');
    expect((result.content[1] as { text: string }).text).toContain('Test error message');
    expect((result.content[1] as { text: string }).text).not.toContain('Test log message');
    expect((result.content[1] as { text: string }).text).not.toContain('Test warning message');
  });
  ...
  test('should retrieve console logs with search filter', async () => {
    ...
    expect(result.isError).toBe(false);
    expect((result.content[0] as { text: string }).text).toContain('Retrieved 1 console log(s)');
    expect((result.content[1] as { text: string }).text).toContain('Test error with [special] characters');
    expect((result.content[1] as { text: string }).text).not.toContain('Test log message');
    expect((result.content[1] as { text: string }).text).not.toContain('Another warning message');
  });
  ...
  test('should retrieve console logs with limit', async () => {
    ...
    expect(result.isError).toBe(false);
    expect((result.content[0] as { text: string }).text).toContain('Retrieved 5 console log(s)');
    ...
    const logText = (result.content[1] as { text: string }).text as string;
  ...
  test('should clear console logs when requested', async () => {
    ...
    expect(result.isError).toBe(false);
    expect((result.content[0] as { text: string }).text).toContain('Retrieved 2 console log(s)');
  ...
  test('should handle no logs', async () => {
    ...
    expect(result.isError).toBe(false);
    expect((result.content[0] as { text: string }).text).toContain('No console logs matching the criteria');
  });
}); 